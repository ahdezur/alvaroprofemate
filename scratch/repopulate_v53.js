...
// Update dbVersion check to 5.3 and check for exercise title in LocalStorage condition
dbJs = dbJs.replace(/dbVersion === "[0-9\.]+"/g, 'dbVersion === "5.3"');
dbJs = dbJs.replace(/localStorage\.setItem\("alvaro_profemate_db_version", "[0-9\.]+"\);/g, 'localStorage.setItem("alvaro_profemate_db_version", "5.3");');
dbJs = dbJs.replace(/chapters\.includes\("Curvas de Nivel y Grafos"\)/g, 'chapters.includes("Determinación de Dominio con Múltiples Restricciones")');
...
if (targetIdx !== -1 && nextElseIdx !== -1) {
  ...
  console.log("Successfully repopulated Chapters 1.1 and 1.2 with ALL EXERCISES AND 3D LAB into db.js (v5.3)!");
}
