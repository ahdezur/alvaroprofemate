...
\begin{teoria}
  ...
  \begin{proof}[\textbf{Demostración:}]
    ...
    $$ |f(x,y) - L| < \dfrac{\varepsilon}{2} $$

    Para cualquier $x$ fijo que cumpla $0 < |x - a| < \dfrac{\delta}{\sqrt{2}}$, consideramos los valores de $y$ que satisfacen $0 < |y - b| < \dfrac{\delta}{\sqrt{2}}$. Para dichos puntos, la distancia euclidiana satisface $\sqrt{(x-a)^2 + (y-b)^2} < \delta$, por lo que la desigualdad del límite se sostiene.

    Tomando el límite cuando $y \to b$ en la desigualdad $|f(x,y) - L| < \dfrac{\varepsilon}{2}$, y aprovechando que $\displaystyle\lim_{y \to b} f(x,y) = g(x)$, obtenemos por propiedades de desigualdades en límites:
    $$ |g(x) - L| \leq \dfrac{\varepsilon}{2} < \varepsilon $$

    Como esto es válido para todo $x$ tal que $0 < |x - a| < \dfrac{\delta}{\sqrt{2}}$, por definición formal tenemos que:
...
\begin{aplicacion}
  ...
  \begin{ejemplo}{Criterio 1: Inexistencia por Caminos (Rectas)}
    Estudie la existencia del límite: $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^2}{x^2 + y^2}$
    
    \textbf{Desarrollo:}
    Tomemos la recta $y = x$, que viene a ser nuestra trayectoria $A_1$:
    $$ \displaystyle\lim_{x \to 0} \dfrac{x^2}{x^2 + x^2} = \displaystyle\lim_{x \to 0} \dfrac{x^2}{2x^2} = \dfrac{1}{2} $$
    
    Tomemos otra recta, digamos el eje X ($y = 0$), la cual viene a ser nuestra trayectoria $A_2$:
    $$ \displaystyle\lim_{x \to 0} \dfrac{x^2}{x^2 + 0} = \displaystyle\lim_{x \to 0} \dfrac{x^2}{x^2} = 1 $$
  ...
  \begin{ejemplo}{Criterio 2: Inexistencia por Límites Iterados}
    Estudie la existencia del límite: $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{y}{y + x^2}$
    
    \textbf{Desarrollo:}
    Calculamos el primer límite iterado (evaluando primero $y \to 0$ y luego $x \to 0$):
    $$ \displaystyle\lim_{x \to 0} \left( \displaystyle\lim_{y \to 0} \dfrac{y}{y + x^2} \right) = \displaystyle\lim_{x \to 0} \left( \dfrac{0}{0 + x^2} \right) = \displaystyle\lim_{x \to 0} (0) = 0 $$
    
    Calculamos el segundo límite iterado (evaluando primero $x \to 0$ y luego $y \to 0$):
    $$ \displaystyle\lim_{y \to 0} \left( \displaystyle\lim_{x \to 0} \dfrac{y}{y + x^2} \right) = \displaystyle\lim_{y \to 0} \left( \dfrac{y}{y + 0} \right) = \displaystyle\lim_{y \to 0} (1) = 1 $$
  ...
  \begin{ejemplo}{Criterio 3: Existencia vía Coordenadas Polares}
    Estudie la existencia del límite: $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^4 + 3(x^2 + y^2)}{x^2 + y^2}$
    
    \textbf{Desarrollo:}
    Aplicamos el cambio a coordenadas polares: $x = r\cos\theta$, $y = r\sin\theta$, sabiendo que $x^2 + y^2 = r^2$.
    Sustituimos en la expresión:
    $$ \displaystyle\lim_{r \to 0^+} \dfrac{(r\cos\theta)^4 + 3r^2}{r^2} = \displaystyle\lim_{r \to 0^+} \dfrac{r^4\cos^4\theta + 3r^2}{r^2} $$
  ...
  \begin{preguntaalternativas}{La Elección del Camino Adecuado}
    ¿Cuál es el argumento correcto respecto al límite $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{xy^2}{x^2 + y^4}$?
    ...
    \opcion{El límite no existe, porque al aproximarse por la curva $x = y^2$ se obtiene $1/2$, mientras que por el eje X ($y=0$) se obtiene $0$.}{Correcto}{¡Brillante! Al sustituir la parábola horizontal $x = y^2$, el límite se convierte en $\displaystyle\lim_{y \to 0} \dfrac{y^2 \cdot y^2}{(y^2)^2 + y^4} = \dfrac{y^4}{2y^4} = \dfrac{1}{2}$. Como $1/2 \neq 0$, el límite se destruye.}
  ...
  \begin{preguntaverdaderofalso}{Conclusión sobre Límites Iterados}
    Considere la función $f(x,y) = \dfrac{xy}{x^2 + y^2}$. Dado que ambos límites iterados son iguales a cero (es decir, $\displaystyle\lim_{x \to 0} (\displaystyle\lim_{y \to 0} f(x,y)) = 0$ y $\displaystyle\lim_{y \to 0} (\displaystyle\lim_{x \to 0} f(x,y)) = 0$), podemos concluir matemáticamente que el límite doble existe y vale cero.
    
    \verdaderofalso{F}
      {¡Excelente! Recordaste la alerta teórica: si los límites iterados son iguales, \textbf{no se puede concluir nada}. De hecho, si evalúas este límite por rectas $y = mx$, el resultado es $\dfrac{m}{1+m^2}$, lo que demuestra que depende de la pendiente $m$ y, por ende, el límite no existe.}
  ...
  \begin{preguntacasillas}{Dependencia Angular en Polares}
    Se requiere evaluar el límite $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \arccos\left(\dfrac{x}{\sqrt{x^2 + y^2}}\right)$. Al realizar el cambio a coordenadas polares, selecciona \textbf{todas} las afirmaciones correctas:
    
    \casilla{La expresión se reduce algebraicamente a $\arccos(\cos\theta)$.}{Correcto}{¡Correcto! Sustituyendo $x = r\cos\theta$ y $\sqrt{x^2+y^2} = r$, nos queda $\arccos(\dfrac{r\cos\theta}{r}) = \arccos(\cos\theta)$.}
  ...
  \begin{pareados}{Asociación: Límite y Herramienta de Análisis}
    ...
    % COLUMNA 1: Límites
    \columnaI{
      \item $\displaystyle \displaystyle\lim_{(x,y)\to(0,0)} \dfrac{x^2y}{x^2+y^2}$
      \item $\displaystyle \displaystyle\lim_{(x,y)\to(0,0)} \dfrac{xy^2}{x^2+y^4}$
      \item $\displaystyle \displaystyle\lim_{(x,y)\to(0,0)} \dfrac{y}{y+x^2}$
    }
...
\begin{ejercicios}
  ...
  \begin{ejercicioresuelto}{Inexistencia por Trayectorias no Lineales}{nivel-3}
    \enunciado{
      ...
      $$ \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^6 y^3}{x^{12} + y^6} $$
    }
    \solucion{
      \textbf{1. Prueba con rectas oblicuas ($y = mx$):}
      Si nos aproximamos por cualquier recta que pase por el origen, sustituimos $y = mx$:
      $$ \displaystyle\lim_{x \to 0} \dfrac{x^6 (mx)^3}{x^{12} + (mx)^6} = \displaystyle\lim_{x \to 0} \dfrac{m^3 x^9}{x^{12} + m^6 x^6} = \displaystyle\lim_{x \to 0} \dfrac{m^3 x^3}{x^6 + m^6} $$
      ...
      $$ \displaystyle\lim_{x \to 0} \dfrac{x^6 (x^2)^3}{x^{12} + (x^2)^6} = \displaystyle\lim_{x \to 0} \dfrac{x^6 \cdot x^6}{x^{12} + x^{12}} = \displaystyle\lim_{x \to 0} \dfrac{x^{12}}{2x^{12}} = \dfrac{1}{2} $$
  ...
  \begin{ejercicioresuelto}{Existencia vía Coordenadas Polares}{nivel-2}
    \enunciado{
      ...
      $$ \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^3}{x^2 + y^2} $$
    }
    \solucion{
      ...
      $$ f(r\cos\theta, r\sin\theta) = \dfrac{(r\cos\theta)^3}{(r\cos\theta)^2 + (r\sin\theta)^2} = \dfrac{r^3\cos^3\theta}{r^2(\cos^2\theta + \sin^2\theta)} = r\cos^3\theta $$
  ...
  \begin{ejerciciopropuesto}{Criterio de Límites Iterados}{nivel-2}
    \enunciado{
      ...
      \begin{enumerate}
        \item $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^2}{x^2 + y^2}$
        \item $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} x\sin\left(\dfrac{1}{x}\right)\sin\left(\dfrac{1}{y}\right)$
      \end{enumerate}
    }
    \pista{
  ...
  \begin{ejerciciopropuesto}{Inexistencia en Coordenadas Polares}{nivel-2}
    \enunciado{
      ...
      $$ \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{(x-y)^2}{x^2 + y^2} $$
    }
    \pista{
  ...
  \begin{ejerciciopropuesto}{Resolución por Inspección Directa (Factorización)}{nivel-1}
    \enunciado{
      ...
      \begin{enumerate}
        \item $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^4 - y^4}{x^2 + y^2}$
        \item $\displaystyle \displaystyle\lim_{(x,y) \to (0,0)} \dfrac{x^3 - y^3}{x^2 + xy + y^2}$
      \end{enumerate}
    }
    \pista{
