...
\begin{teoria}
  ...
  \begin{teorema}{Álgebra de Límites}
    ...
    \begin{enumerate}
      ...
      \item \textbf{Cociente:} $\displaystyle \lim_{(x,y) \to (a,b)} \left[ \frac{f(x,y)}{g(x,y)} \right] = \frac{L}{M}$, siempre que $M \neq 0$.
    \end{enumerate}
  \end{teorema}
  ...
  \begin{proof}[\textbf{Demostración:}]
    ...
    Por otro lado, como $\displaystyle \lim_{(x,y) \to (a,b)} f(x,y) = 0$, para todo $\varepsilon > 0$ existe $\delta_2 > 0$ tal que si $0 < \sqrt{(x-a)^2 + (y-b)^2} < \delta_2$, entonces $|f(x,y)| < \frac{\varepsilon}{M}$.
...
\begin{aplicacion}
  ...
  \begin{preguntaalternativas}{Aplicación de Teoremas de Acotamiento}
    Para calcular $\displaystyle \lim_{(x,y) \to (0,0)} y^2 \cos\left(\frac{1}{xy}\right)$, ¿cuál es el argumento analítico correcto?
  ...
  \begin{pareadostrescolumnas}{Asociación: Límite, Técnica Analítica y Resultado}
    ...
    \columnaI{
      ...
      \item $\displaystyle \lim_{(x,y)\to(0,0)} x^2 \arctan\left(\frac{1}{y}\right)$
      \item $\displaystyle \lim_{(x,y)\to(3,3)} \frac{x^2-y^2}{x-y}$
      \item $\displaystyle \lim_{(x,y)\to(0,0)} \frac{1-\cos(x^2+y^2)}{(x^2+y^2)^2}$
    }
    ...
    \pareotres{1-E-IV}{¡Correcto! Usando la sustitución $u = x^2+y^2$, el límite se transforma en el notable $\displaystyle \lim_{u\to0} \frac{\sin(u)}{u} = 1$.}
    ...
    \pareotres{5-C-V}{¡Magistral! Haciendo el cambio $u = x^2+y^2$, obtenemos el límite notable canónico $\displaystyle \lim_{u\to0} \frac{1-\cos(u)}{u^2} = \frac{1}{2}$.}
...
\begin{ejercicios}
  ...
  \begin{ejerciciopropuesto}{Límite Combinado de Seno y Raíz}{nivel-2}
    ...
    \pista{
      Puedes reescribir la expresión reordenando sus términos como $x^2 \cdot \frac{\sin(x^2 + y^2)}{x^2 + y^2} \cdot \sqrt{x^2 + y^2}$. Utiliza el límite notable $\displaystyle \lim_{u \to 0} \frac{\sin(u)}{u} = 1$ haciendo $u = x^2 + y^2$ junto con el Álgebra de Límites, o bien acota utilizando que $|\sin(u)| \leq 1$ para aplicar Cero por Acotado.
    }
