...
\begin{aplicacion}
  ...
  \begin{procesamiento}{Laboratorio 3D Interactivo: Planos de Corte y Curvas de Nivel}
  ...
  \begin{preguntacasillas}{Propiedades de Campos Racionales}
    Considera la función $f(x,y) = \dfrac{y}{x}$. Selecciona \textbf{todas} las afirmaciones matemáticamente correctas respecto a su dominio y curvas de nivel:
    ...
    \casilla{Las curvas de nivel $f(x,y) = k$ corresponden a rectas que pasan por el origen (excluyendo el punto $(0,0)$).}{Correcto}{Correcto. La ecuación $\dfrac{y}{x} = k \implies y = kx$ representa rectas de pendiente $k$, pero como $x \neq 0$, el origen no pertenece al dominio.}
  ...
  \begin{pareadostrescolumnas}{Asociación Avanzada: Función, Geometría de $C_k$ y Restricción de $k$}
    ...
    \pareotres{5-C-IV}{¡Magistral! Al igualar $\dfrac{-1}{x^2 + y^2} = k \implies x^2 + y^2 = -\dfrac{1}{k}$, obtenemos circunferencias. Como $x^2 + y^2$ es estrictamente positivo en su dominio (excluyendo el origen), la fracción completa es estrictamente negativa, por lo que $k < 0$.}
...
\begin{ejercicios}
  ...
  \begin{ejercicioresuelto}{Círculos Tangentes y Completación de Cuadrados}{nivel-3}
    ...
    \solucion{
      ...
      Al evaluar a lo largo del eje Y (donde $x=0$), la función se reduce a $f(0,y) = \dfrac{2y}{y^2} = \dfrac{2}{y}$. Como $y$ puede ser cualquier valor real distinto de cero, la fracción $\dfrac{2}{y}$ puede tomar cualquier valor real, por lo tanto, $\operatorname{im}(f) = \mathbb{R}$.
      ...
      \textit{Caso A ($k = 0$):}
      $$ \dfrac{2y}{x^2 + y^2} = 0 \implies 2y = 0 \implies y = 0 $$
      ...
      \textbf{Conclusión:}
      Para cada $k \neq 0$, la curva de nivel $C_k$ es una \textbf{circunferencia} de radio $R = \dfrac{1}{|k|}$, centrada en el punto $(0, \dfrac{1}{k})$. Notemos que todas estas circunferencias pasan por el origen (ya que $0^2 + (0 - 1/k)^2 = 1/k^2$), pero el origen $(0,0) \notin \operatorname{dom}(f)$, por lo que es un punto excluido ("agujero") en cada una de ellas.
    }
  ...
  \begin{ejerciciopropuesto}{Cónicas y Combinación Lineal}{nivel-2}
    ...
    \pista{
      Aquí $\operatorname{im}(h) = [0, \infty)$. Al igualar $4x^2 + 9y^2 = k$, deberás dividir toda la ecuación por $k$ para llevarla a la forma canónica $\dfrac{x^2}{a^2} + \dfrac{y^2}{b^2} = 1$. Verás que para $k>0$ el mapa de contorno corresponde a elipses centradas en el origen, donde su eje mayor descansa sobre el eje X y su eje menor sobre el eje Y. Para $k=0$, obtendrás un único punto.
    }
  \end{ejerciciopropuesto}
