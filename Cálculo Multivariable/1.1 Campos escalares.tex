...
\begin{teoria}
  ...
  \begin{procesamiento}{Restricciones Clásicas de Dominio}
    Para determinar analíticamente el dominio natural de un campo escalar, debes buscar y plantear las mismas restricciones algebraicas que conoces de funciones reales: \begin{itemize}
      \item Denominadores: Deben ser distintos de cero ($g(\vec{x}) \neq 0$).
      \item Raíces de índice par: El radicando debe ser no negativo ($g(\vec{x}) \geq 0$).
      \item Logaritmos: El argumento debe ser estrictamente positivo ($g(\vec{x}) > 0$).
    \end{itemize}
...
\begin{aplicacion}
  ...
  % --- TIPO 2 : PREGUNTA DE VERDADERO O FALSO (Concepto teórico) ---
...
\verdaderofalso{F}{Incorrecto. Recuerda la palabra 'escalar'. La salida es siempre un único número real (como una temperatura o una altitud), no un vector.}{¡Correcto! Entregamos un punto o vector de entrada $(x,y,z)$, pero la función nos devuelve un escalar (un único número real).}
  ...
  % --- TIPO 3 : PREGUNTA DE CASILLAS (Combinación de restricciones) ---
  \begin{preguntacasillas}{Múltiples Restricciones}
    Considera la función $h(x,y) = \dfrac{\sqrt{x+y}}{x-1}$. Selecciona \textbf{todas} las condiciones matemáticas que deben cumplirse simultáneamente para que un punto $(x,y)$ pertenezca a su dominio natural : \casilla{$x + y \geq 0$}{Correcto}{Correcto. Al ser una raíz cuadrada en el numerador, su interior no puede ser negativo, pero sí puede ser cero.}
  ...
  \begin{pareadostrescolumnas}{Asociación Avanzada : Función, Dominio e Imagen}
...
\begin{ejercicios}
  ...
  \begin{ejercicioresuelto}{Determinación de Dominio con Múltiples Restricciones}{nivel-2}
    \enunciado{
      ...
      $$f(x,y) = \dfrac{\sqrt{9 - x^2 - y^2}}{\ln(y - x)}$$
    }
    \solucion{
  ...
  \begin{ejerciciodemostracion}{Invarianza por Simetría Radial}{nivel-3}
    ...
    \demostracion{
      ...
      \begin{enumerate}
        ...
        \item Como $\displaystyle \lim_{t \to \infty} \ln(1 + t^2) = \infty$ y la función es continua, por el Teorema del Valor Intermedio el recorrido toma todos los valores intermedios.
      \end{enumerate}
