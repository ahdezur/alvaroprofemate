...
\begin{motivacion}
  ...
  \begin{preguntaguia}
    Si un campo escalar $f(x,y)$ no está definido en el origen $(0,0)$, pero sabemos que el límite $\displaystyle \lim_{(x,y)\to(0,0)} f(x,y) = 5$, ¿qué valor exacto debemos asignarle a $f(0,0)$ para transformarla en una función continua en todo el plano?
  \end{preguntaguia}
\end{motivacion}
...
\begin{teoria}
  ...
  \begin{definicion}{Continuidad de un Campo Escalar en un Punto}
    ...
    \begin{enumerate}
      \item $(x_0, y_0) \in \operatorname{dom}(f)$ (El punto está bien definido en la función).
      \item Existe el límite $\displaystyle \lim_{(x,y) \to (x_0, y_0)} f(x,y) = L \in \mathbb{R}$.
      \item $L = f(x_0, y_0)$ (El límite coincide con el valor evaluado).
    \end{enumerate}
  \end{definicion}
  ...
  \begin{ejemplo}{Uso de Sucesiones para Descartar Continuidad}
    ...
    Consideremos la sucesión de puntos $(x_n, y_n) = \left(\dfrac{1}{n}, \dfrac{1}{n}\right)$.
    Es evidente que cuando $n \to \infty$, la sucesión converge al origen: $\displaystyle \lim_{n \to \infty} \left(\dfrac{1}{n}, \dfrac{1}{n}\right) = (0,0)$.
    
    Evaluamos la función en los términos de la sucesión:
    $$ f\left(\dfrac{1}{n}, \dfrac{1}{n}\right) = \dfrac{\left(\dfrac{1}{n}\right)\left(\dfrac{1}{n}\right)}{\left(\dfrac{1}{n}\right)^2 + \left(\dfrac{1}{n}\right)^2} = \dfrac{\dfrac{1}{n^2}}{\dfrac{2}{n^2}} = \dfrac{1}{2} $$
    
    Tomando el límite cuando $n \to \infty$, obtenemos:
    $$ \lim_{n \to \infty} f\left(\dfrac{1}{n}, \dfrac{1}{n}\right) = \dfrac{1}{2} $$
  ...
  \begin{definicion}{Tipos de Discontinuidad}
    ...
    \begin{itemize}
      \item \textbf{Discontinuidad Reparable (o Evitable):} Ocurre cuando el límite existe ($\displaystyle \lim_{(x,y) \to (x_0, y_0)} f(x,y) = L \in \mathbb{R}$), pero se cumple que:
  ...
  \begin{teorema}{Propiedades de las Funciones Continuas}
    ...
    \begin{itemize}
      \item \textbf{Álgebra:} Las funciones $f \pm g$, $f \cdot g$ y $\dfrac{f}{g}$ (si $g(x_0, y_0) \neq 0$) son continuas en $(x_0, y_0)$.
...
\begin{aplicacion}
  ...
  \begin{preguntaalternativas}{Reparando el Espacio}
    Considere el campo escalar $f(x,y) = \dfrac{\sin(xy)}{xy}$. Sabemos que la función no está definida en los ejes coordenados (donde $x=0$ o $y=0$). Si analizamos el comportamiento exclusivamente en el origen $(0,0)$, ¿cuál de las siguientes afirmaciones es matemáticamente correcta?
    ...
    \opcion{Presenta una discontinuidad irreparable, ya que al acercarnos por el eje X ($y=0$) la función se indefine y por tanto el límite no existe.}{Incorrecto}{Cuidado, el límite sí existe. Al hacer la sustitución $u = xy$, cuando $(x,y) \to (0,0)$, la variable $u \to 0$. El límite es $\displaystyle \lim_{u\to 0}\dfrac{\sin(u)}{u} = 1$.}
  ...
  \begin{preguntaverdaderofalso}{La Falsa Sensación de Continuidad}
    ...
    Un estudiante argumenta lo siguiente: \textit{"Dado que $f(0,0) = 0$, y al acercarnos por el eje X ($y=0$) obtenemos $\displaystyle \lim_{x\to 0} \dfrac{0}{x^2} = 0$, y por el eje Y ($x=0$) obtenemos $\displaystyle \lim_{y\to 0} \dfrac{0}{y^2} = 0$, los límites coinciden con la función evaluada. Por lo tanto, $f$ es continua en el origen."}
    
    \verdaderofalso{F}
      {¡Correcto! Has detectado la "trampa de los ejes". Acercarse por los ejes no es suficiente en $\mathbb{R}^2$. Si te acercas por la recta diagonal $y = x$, el límite es $\displaystyle \lim_{x\to 0}\dfrac{x^2}{2x^2} = \dfrac{1}{2}$. Como $1/2 \neq 0$, el límite global no existe, lo que significa que estamos ante una \textbf{discontinuidad irreparable} (esencial), a pesar de que la función valga cero en el origen.}
  ...
  \begin{pareados}{Clasificando Discontinuidades}
    ...
    % COLUMNA 1: Funciones
    \columnaI{
      \item $\displaystyle f(x,y) = \dfrac{\sin(x^2+y^2)}{x^2+y^2}$ (no definida en el origen)
      \item $\displaystyle f(x,y) = \begin{cases} \dfrac{x^2}{x^2+y^2} & (x,y) \neq (0,0) \\ 0 & (x,y) = (0,0) \end{cases}$
      \item $\displaystyle f(x,y) = \begin{cases} \dfrac{xy^2}{x^2+y^2} & (x,y) \neq (0,0) \\ 0 & (x,y) = (0,0) \end{cases}$
      \item $\displaystyle f(x,y) = \dfrac{x^2-y^2}{x^2+y^2}$ (no definida en el origen)
      \item $\displaystyle f(x,y) = \dfrac{1-\cos(x^2+y^2)}{(x^2+y^2)^2}$ (no definida en el origen)
    }
...
\begin{ejercicios}
  ...
  \begin{ejercicioresuelto}{Continuidad con Parámetros $\alpha$ y $\beta$}{nivel-3}
    \enunciado{
      ...
      $$ f(x,y) = \begin{cases} \dfrac{(xy)^\alpha}{(x^2+y^2)^\beta} & \text{si } (x,y) \neq (0,0) \\[0.5em] 0 & \text{si } (x,y) = (0,0) \end{cases} $$
      Muestre formalmente que $f$ es continua en el origen si y solo si $\alpha > \beta$.
    }
    \solucion{
      Para que $f$ sea continua en el origen, debe cumplirse que $\displaystyle \lim_{(x,y)\to(0,0)} f(x,y) = f(0,0) = 0$.
      ...
      $$ \left| \dfrac{(\rho\cos\theta \cdot \rho\sin\theta)^\alpha}{(\rho^2)^\beta} \right| = \dfrac{\rho^{2\alpha} |\cos\theta\sin\theta|^\alpha}{\rho^{2\beta}} = \rho^{2(\alpha-\beta)} |\cos\theta\sin\theta|^\alpha $$
  ...
  \begin{ejercicioresuelto}{Redefinición y Extensión por Continuidad}{nivel-2}
    \enunciado{
      Estudie el límite $\displaystyle \lim_{(x,y)\to(0,0)} f(x,y)$ para cada una de las siguientes funciones. En base a su respuesta, decida si es posible dar un valor a $f(0,0)$ de tal manera que la función quede continua en $(0,0)$ (discontinuidad reparable).
      
      \begin{enumerate}
        \item $\displaystyle f(x,y) = \dfrac{\tan(x^2+y^2)}{x^2+y^2}$
        \item $\displaystyle f(x,y) = \dfrac{\tan(xy)}{y}$
        \item $\displaystyle f(x,y) = \dfrac{x}{y}$
        \item $\displaystyle f(x,y) = \dfrac{xy+y^3}{x^2+y^2}$
      \end{enumerate}
    }
    \solucion{
      \begin{enumerate}
        \item \textbf{Análisis:} Haciendo el cambio $u = x^2+y^2$, tenemos $\displaystyle \lim_{u\to 0}\dfrac{\tan u}{u} = 1$. El límite existe (inspección directa y límites notables).\\
        \textbf{Conclusión:} Discontinuidad reparable. Es posible hacerla continua definiendo $f(0,0) = 1$.
        
        \item \textbf{Análisis:} Multiplicando y dividiendo por $x$, obtenemos $x \cdot \dfrac{\tan(xy)}{xy}$. Usando el límite notable $\tan(u)/u \to 1$, nos queda $\displaystyle \lim_{(x,y)\to(0,0)} (x \cdot 1) = 0$. El límite existe.\\
        \textbf{Conclusión:} Discontinuidad reparable. Es posible hacerla continua definiendo $f(0,0) = 0$.
        
        \item \textbf{Análisis:} Evaluando por la familia de rectas $y = mx$, el límite resulta $\dfrac{x}{mx} = \dfrac{1}{m}$. Como el resultado depende de la pendiente $m$, el límite no existe.\\
        \textbf{Conclusión:} Discontinuidad irreparable. No es posible dar un valor a $f(0,0)$.
        
        \item \textbf{Análisis:} Pasando a polares, obtenemos $\dfrac{\rho^2\cos\theta\sin\theta + \rho^3\sin^3\theta}{\rho^2} = \cos\theta\sin\theta + \rho\sin^3\theta$. Cuando $\rho \to 0$, el límite es $\cos\theta\sin\theta$, el cual depende del ángulo de aproximación.\\
        \textbf{Conclusión:} Discontinuidad irreparable. No es posible dar un valor a $f(0,0)$.
      \end{enumerate}
    }
  \end{ejercicioresuelto}
  ...
  \begin{ejercicioresuelto}{Continuidad en Regiones Definidas a Trozos}{nivel-3}
    \enunciado{
      ...
      $$ f(x,y) = \begin{cases} \alpha(x^2+y^2) & \text{si } x^2+y^2 < 2 \\[0.5em] \dfrac{1}{\beta\sqrt{x^2+y^2}} & \text{si } x^2+y^2 \ge 2 \end{cases} $$
      \begin{itemize}
    ...
    \solucion{
      ...
      $$ \lim_{(x,y) \to (x_0,y_0)^+} \dfrac{1}{\beta\sqrt{x^2+y^2}} = \dfrac{1}{\beta\sqrt{2}} $$
      
      Para garantizar la continuidad (ausencia de salto en la frontera), ambos límites deben ser idénticos:
      $$ 2\alpha = \dfrac{1}{\beta\sqrt{2}} \implies \alpha\beta = \dfrac{1}{2\sqrt{2}} = \dfrac{\sqrt{2}}{4} $$
      Por lo tanto, la relación exigida es $\alpha\beta = \dfrac{\sqrt{2}}{4}$.
    }
  \end{ejercicioresuelto}
...
\begin{formulas}
    ...
    \formula{Condiciones de Continuidad en $(x_0, y_0)$}
      {\\begin{array}{l}
       1.\; (x_0,y_0) \in \operatorname{dom}(f) \\[0.4em]
       2.\; \exists \displaystyle \lim_{(x,y)\to(x_0,y_0)} f(x,y) \\[0.4em]
