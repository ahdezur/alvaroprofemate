...
\begin{teoria}
  ...
  \begin{ejemplo}{Uso de Sucesiones para Descartar Continuidad}
    ...
    Es evidente que cuando $n \to \infty$, la sucesión converge al origen: $\displaystyle \lim_{n \to \infty} \left(\frac{1}{n}, \frac{1}{n}\right) = (0,0)$.
...
\begin{aplicacion}
  ...
  \begin{preguntaalternativas}{Reparando el Espacio}
    ...
    \opcion{Presenta una discontinuidad irreparable, ya que al acercarnos por el eje X ($y=0$) la función se indefine y por tanto el límite no existe.}{Incorrecto}{Cuidado, el límite sí existe. Al hacer la sustitución $u = xy$, cuando $(x,y) \to (0,0)$, la variable $u \to 0$. El límite es $\displaystyle \lim_{u\to 0}\frac{\sin(u)}{u} = 1$.}
  ...
  \begin{preguntaverdaderofalso}{La Falsa Sensación de Continuidad}
    ...
    Un estudiante argumenta lo siguiente: \textit{"Dado que $f(0,0) = 0$, y al acercarnos por el eje X ($y=0$) obtenemos $\displaystyle \lim_{x\to 0} \frac{0}{x^2} = 0$, y por el eje Y ($x=0$) obtenemos $\displaystyle \lim_{y\to 0} \frac{0}{y^2} = 0$, los límites coinciden con la función evaluada. Por lo tanto, $f$ es continua en el origen."}
    
    \verdaderofalso{F}
      {¡Correcto! Has detectado la "trampa de los ejes". Acercarse por los ejes no es suficiente en $\mathbb{R}^2$. Si te acercas por la recta diagonal $y = x$, el límite es $\displaystyle \lim_{x\to 0}\frac{x^2}{2x^2} = \frac{1}{2}$. Como $1/2 \neq 0$, el límite global no existe, lo que significa que estamos ante una \textbf{discontinuidad irreparable} (esencial), a pesar de que la función valga cero en el origen.}
...
\begin{ejercicios}
  ...
  \begin{ejercicioresuelto}{Redefinición y Extensión por Continuidad}{nivel-2}
    ...
    \solucion{
      \begin{enumerate}
        \item \textbf{Análisis:} Haciendo el cambio $u = x^2+y^2$, tenemos $\displaystyle \lim_{u\to 0}\frac{\tan u}{u} = 1$. El límite existe (inspección directa y límites notables).\\
        \textbf{Conclusión:} Discontinuidad reparable. Es posible hacerla continua definiendo $f(0,0) = 1$.
        
        \item \textbf{Análisis:} Multiplicando y dividiendo por $x$, obtenemos $x \cdot \frac{\tan(xy)}{xy}$. Usando el límite notable $\tan(u)/u \to 1$, nos queda $\displaystyle \lim_{(x,y)\to(0,0)} (x \cdot 1) = 0$. El límite existe.\\
